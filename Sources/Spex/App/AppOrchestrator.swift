    ...
    func runGenerate(args: GenerateCommand.Arguments) async throws {
        ...
        var spec: Specification = try await noora.progressStep(message: "Loading and validating specification") { _ in
            ...
            if loadedSpec.outputDatasets == nil || loadedSpec.outputDatasets!.isEmpty {
                Logger.log(
                    "No output dataset provided. Providing a sample output can improve generation quality.",
                    type: .warning)
            }
            return loadedSpec
        }
        ...
        // --- 6. Generate Code ---
        let generatedCode = try await noora.progressStep(
            message: "Generating project code via \(provider.rawValue.capitalized) (\(model))"
        ) { _ in
        ...
        if fileManager.fileExists(atPath: fileURL.path) {
            throw AppError.fileWriteError(
                ...
                source: NSError(
                    ...
                    code: 1,
                    userInfo: [
                        NSLocalizedDescriptionKey: "A file named '\(specFilename)' already exists in this directory."
                    ]
                )
            )
        }
            ...
            for (index, dataset) in enrichedInputDatasets.enumerated() {
                group.addTask {
                    guard let dataSource = DataSourceFactory.makeDataSource(for: dataset) else {
                        throw AppError.inputError(
                            "Input dataset '\(dataset.name)' is missing a 'sample_data_path' or 'sample_data_block'.")
                    }
            ...
            try await withThrowingTaskGroup(of: (Int, String).self) { group in
                for (index, dataset) in outputDatasets.enumerated() {
                    group.addTask {
                        guard let dataSource = DataSourceFactory.makeDataSource(for: dataset) else {
                            throw AppError.inputError(
                                "Output dataset '\(dataset.name)' is missing a 'sample_data_path' or 'sample_data_block'."
                            )
                        }
                        let schemaAndSample = try await dataSource.fetchSchemaAndSample(for: dataset)
                        return (index, schemaAndSample)
...
extension AppOrchestrator {
    ...
    func runInteractive() async throws {
        ...
        let startProject = noora.singleChoicePrompt(
            ...
            options: hasExistingSpec ? ["Use existing spec.toml", "Create new spec.toml", "Exit"] : ["Yes", "No"]
        )
        ...
        let analysisType = noora.textPrompt(
            title: "Project Setup",
            prompt:
                "What type of analysis are you building? (e.g., Ad Attribution, User Segmentation, Revenue Analysis)",
