...
final class OpenAIClient: LlmClient {
    ...
    init(settings: Settings, urlSession: URLSession = .shared) throws {
        guard let apiKey = settings.openaiApiKey else {
            throw AppError.configError("OPENAI_API_KEY not found in environment or .env file. It is required for the OpenAI provider.")
        }
        self.apiKey = apiKey
    ...
    func generateCode(prompt: String, model: String) async throws -> String {
        ...
        guard (200...299).contains(httpResponse.statusCode) else {
            ...
            throw AppError.llmError("OpenAI API returned non-success status: \(httpResponse.statusCode) - \(errorBody)")
        }
