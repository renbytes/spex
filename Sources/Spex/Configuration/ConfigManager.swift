...
struct ConfigManager {
    ...
    private func readString(from url: URL) throws -> String {
        do {
            ...
            throw AppError.fileReadError("Failed to read template at '\(url.path)': \(error.localizedDescription)")
        }
    }
    ...
    func getReadmeInstructions(language: String, instructionType: String) -> String {
        ...
        let path = configRoot.appendingPathComponent("\(language.lowercased())/instructions_\(instructionType).md")
        return (try? readString(from: path)) ?? ""
    }
