...
final class WriterTests: XCTestCase {
    ...
    func testWriteFilesCreatesCorrectDirectoryAndContent() throws {
        ...
        // ASSERT
        let generatedDirs = try FileManager.default.contentsOfDirectory(at: tempDirectory.appendingPathComponent("generated_jobs/python/simple-test"), includingPropertiesForKeys: nil)
        ...
        XCTAssertEqual(jobContent.trimmingCharacters(in: .whitespacesAndNewlines), "# This is the job file.\nprint(\"hello from job\")")
        XCTAssertEqual(testContent.trimmingCharacters(in: .whitespacesAndNewlines), "# This is the test file.\nassert True")
