    ...
    init(settings: Settings, urlSession: URLSession = .shared) throws {
        guard let apiKey = settings.geminiApiKey else {
            throw AppError.configError(
                "GEMINI_API_KEY not found in environment or .env file. It is required for the Gemini provider."
            )
        }
        self.apiKey = apiKey
    ...
    func generateCode(prompt: String, model: String) async throws -> String {
        ...
        guard (200...299).contains(httpResponse.statusCode) else {
            ...
            throw AppError.llmError(
                "Gemini API returned non-success status: \(httpResponse.statusCode) - \(errorBody)")
        }
